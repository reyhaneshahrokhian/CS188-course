...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        best_action = [action for action in legal_actions if self.getQValue(state, action) == max_qvalue]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for key in featureVector.keys():
            q_value += self.weights[key] * featureVector[key]
        return q_value
    ...
    def update(self, state, action, nextState, reward):
        ...
        q_value_nxt = self.computeValueFromQValues(nextState)
        for key in feature.keys():
            self.weights[key] += self.alpha * (reward + self.discount * q_value_nxt - q_value_current) * feature[key]
